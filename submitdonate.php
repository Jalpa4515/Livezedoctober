...
if ($res->campaign_typeId == 3) {
    $smstext = 'You have got one donation request on ZedAid campaign '.$shortTitle;
}else{
    $smstext = 'You have got one donation request on ZedAid campaign '.$shortTitle;
}
curl_setopt_array($curl, array(
    ...
    CURLOPT_POSTFIELDS => "{\"sender\":\"ZEDAID\",\"route\":\"1\",\"country\":\"91\",\"DLT_TE_ID\":\"1307163394028199382\",\"sms\":[{\"message\":\"$smstext\",\"to\":[\"$phone\"]}]}",
...
$phone2 = '9892489040';
if ($res->campaign_typeId == 3) {
    $smstextAdmin = 'ZedAid got one donation request on Campaign '.$shortTitle;
}else{
    $smstextAdmin = 'ZedAid got one donation request on Campaign '.$shortTitle;
}
$curl2 = curl_init();
...
curl_setopt_array($curl2, array(
    ...
    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
    CURLOPT_CUSTOMREQUEST => "POST",
    CURLOPT_POSTFIELDS => "{\"sender\":\"ZEDAID\",\"route\":\"1\",\"country\":\"91\",\"DLT_TE_ID\":\"1307163394041208762\",\"sms\":[{\"message\":\"$smstextAdmin\",\"to\":[\"$phone2\"]}]}",
    CURLOPT_HTTPHEADER => array(
        "authkey: 328268AQTjWikZLti61725bd1P1",
        "cache-control: no-cache",
        "content-type: application/json",
...
if ($payment_type == '2') {
    $sql2 = $wpdb->prepare(
       ...
       (campaign_Id, campaign_typeId, anonymous, fullName, emailId, phonenumber, address, amount,payment_type,`status`) 
 values ('" . $campaign_Id . "','" . $campaign_typeId . "','" . $anonymous . "','" . $fullName . "','" . $emailId . "','" . $phonenumber . "','" . $address . "','" . $amount . "','2','1')"
    );
    $wpdb->query($sql2);
}else{
    $sql2 = $wpdb->prepare(
        "INSERT INTO `wp_campaigndonations`      
       (campaign_Id, campaign_typeId, anonymous, fullName, emailId, phonenumber, address, amount,payment_type) 
 values ('" . $campaign_Id . "','" . $campaign_typeId . "','" . $anonymous . "','" . $fullName . "','" . $emailId . "','" . $phonenumber . "','" . $address . "','" . $amount . "','1')"
    );
    $wpdb->query($sql2);
}
